...
bool Read_png(Pixel*& data,int& width,int& height,const char* filename)
{
    ...
    if(num_read!=sizeof header){fclose(file); return false;}
    int ret_sig=png_sig_cmp((png_bytep)header, 0, sizeof header);
    if(ret_sig){fclose(file); return false;}
    png_structp png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, 0, 0, 0);
    if(!png_ptr){fclose(file); return false;}
    png_infop info_ptr = png_create_info_struct(png_ptr);
    if(!info_ptr){fclose(file); return false;}
    png_infop end_info = png_create_info_struct(png_ptr);
    if(!end_info){ fclose(file); return false;}
    png_init_io(png_ptr, file);
