...
void Parse(Render_World& world, int& width, int& height, const char* test_file)
{
    ...
    while (fgets(buff, sizeof (buff), F)) {
        ...
        if(item == "size") {
            ...
            ss >> name >> u >> v >> s0;
            assert(ss);
            Object* o = new Plane(u, v.normalized());
        ...
        } else if(item == "boundedplane") {
            ss >> name >> u >> v >> s0 >> u1 >> v1;
            assert(ss);
            Object* o = new BoundedPlane(u, v.normalized(),u1,v1);
            std::map<std::string, Shader*>::const_iterator sh = shaders.find(s0);
            assert(sh != shaders.end());
            o->material_shader = sh->second;
            if(name == "-") world.objects.push_back(o);
            ...
            std::map<std::string, Shader*>::const_iterator sh = shaders.find(s0);
            assert(sh != shaders.end());
            shaders[name] = new Refractive_Shader(world, sh->second, f0,f1);
